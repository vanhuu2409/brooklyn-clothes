...
const Carousel = () => {
  ...
  const [slideView, setSlideView] = useState(300);

  const handlePrevSlide = () => {
    setSlideView(slideView - 100);
    slideView < 0 && setSlideView((slides.length - 1) * 100);
  };
  const handleNextSlide = () => {
    setSlideView(slideView + 100);
    slideView >= 300 && setSlideView(0);
  };

  return (
    <div className='relative overflow-hidden min-h-[15rem] w-full'>
      {/* slider */}
      <div
        className={`flex w-[400vw] -translate-x-[${slideView}vw] transition-all duration-500`}
      >
        {/* slide item */}
        {slides.map((item) => (
          <div key={item?.id} className='relative'>
            ...
            <div className='top-1/2 left-1/2 lg:gap-6 absolute min-h-[15rem] flex flex-col items-center justify-center gap-2 w-2/3 sm:gap-2 -translate-x-1/2 -translate-y-1/2'>
              ...
              <Link
                to='/'
                className='group/shopall w-fit hover:bg-opacity-100 hover:border bg-opacity-60 inline-flex items-center gap-2 px-5 py-4 mt-4 text-white transition-all bg-black'
              >
      ...
      {/* controls */}
      {/* chev left */}
      <div className='top-1/2 inset-x-0 *:px-6 *:block sm:*:px-6 md:*:px-8 lg:*:px-12 xl:*:px-24 absolute h-full flex justify-between *:text-white *:p-4 *:-m-4 -translate-y-1/2'>
        <button
          onClick={handlePrevSlide}
          className='hover:translate-x-2 sm:hover:-translate-x-4 h-full transition-all'
        >
          <svg
            xmlns='http://www.w3.org/2000/svg'
            fill='none'
